...
import { NotSupportedYetError, TranspileFailedError } from '../utils/errors.ts';
import { error } from '../utils/formatting.ts';
import { createElementaryConversionCall } from '../utils/functionGeneration.ts';
import { createNumberLiteral } from '../utils/nodeTemplates.ts';
import { getParameterTypes, intTypeForLiteral, safeGetNodeType } from '../utils/nodeTypeProcessing.ts';
import { typeNameFromTypeNode, isExternalCall } from '../utils/utils.ts';
...
export class ImplicitConversionToExplicit extends ASTMapper {
  ...
  visitFunctionCall(node: FunctionCall, ast: AST): void {
    ...
    if (node.vFunctionCallType === ExternalReferenceType.Builtin) {
      ...
      if (['node:assert', 'require'].includes(node.vFunctionName) && node.vArguments.length > 1) {
...
function getLiteralValueBound(typeString: string): string {
  ...
  if (BigInt(maxBound.toString()) < BigInt(newTypeString)) {
    // solidity doesn't support literals larger than 256 bits
    return maxBound.toString();
  }

  if (BigInt(minBound.toString()) > BigInt(newTypeString)) {
