...
import { CairoAssert, CairoTempVarStatement } from '../ast/cairoNodes/export.ts';
import { ASTMapper } from '../ast/mapper.ts';
import { printNode } from './astPrinter.ts';
import { InsaneASTError } from './errors.ts';
import { safeGetNodeType } from './nodeTypeProcessing.ts';
import { isNameless } from './utils.ts';
...
export function checkSane(unit: SourceUnit, ctx: ASTContext): void {
  for (const node of unit.getChildren(true)) {
    if (!inCtx(node, ctx)) {
      throw new InsaneASTError(
        `Child in different context: ${ctx.id} from expected ${ctx.id}`,
      );
    }
    ...
    if (node instanceof SourceUnit) {
      for (const [name, symId] of node.exportedSymbols) {
        ...
        if (symNode !== node.vExportedSymbols.get(name)) {
          throw new InsaneASTError(
            `Exported symbol ${name} for id ${symId}  doesn't match vExportedSymbols entry: `,
          );
        }
      }
      ...
      for (let i = 0; i < node.vSymbolAliases.length; i++) {
        ...
        if (!inCtx(def, ctx)) {
          throw new InsaneASTError(
            `Imported symbol  from import  not in expected context `,
          );
        }
      }
      ...
      if (node.vScope !== node.parent) {
        throw new InsaneASTError(
          `Contract  vScope  and parent  differ`,
        );
      }
