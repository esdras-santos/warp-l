...
import { printNode, printTypeNode } from './astPrinter.ts';
import { TranspileFailedError } from './errors.ts';
import { error } from './formatting.ts';
import { getContainingSourceUnit } from './utils.ts';
import { getNodeType, getNodeTypeInCtx } from './typeStrings/typeStringParser.ts';
import { CairoAssert } from '../ast/cairoNodes/export.ts'; // eslint-disable-line
...
export function getParameterTypes(functionCall: FunctionCall, ast: AST): TypeNode[] {
  ...
  switch (functionCall.kind) {
    case FunctionCallKind.FunctionCall:
      ...
      return (functionType as FunctionLikeType).parameters;
    ...
    case FunctionCallKind.StructConstructorCall: {
      ...
      const structDef = (((functionType as TypeNameType).type as PointerType).to as UserDefinedType).definition;
      assert(structDef instanceof StructDefinition);
      return (structDef as StructDefinition).vMembers.map(ast.inference.variableDeclarationToTypeNode, ast.inference);
    }
...
export function isDynamicArray(type: TypeNode): boolean {
  return (
    ...
    (type instanceof ArrayType && (type as ArrayType).size === undefined) ||
...
export function getPackedByteSize(type: TypeNode, inference: InferType): number | bigint {
  ...
  if (type instanceof FixedBytesType) {
    return (type as FixedBytesType).size;
  }
  ...
  if (type instanceof ArrayType && (type as ArrayType).size !== undefined) {
    return BigInt((type as ArrayType).size as unknown as number) * BigInt(getPackedByteSize(type.elementT, inference));
  }
...
export function getByteSize(type: TypeNode, inference: InferType): number | bigint {
  ...
  if (type instanceof ArrayType) {
    assert((type as ArrayType).size !== undefined);
    
    return BigInt((type as ArrayType).size as unknown as number) * BigInt(getByteSize(type.elementT, inference));
  }
