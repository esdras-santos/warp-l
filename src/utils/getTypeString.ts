...
import { RationalLiteral } from '../passes/literalExpressionEvaluator/rationalLiteral.ts';
import { printNode, printTypeNode } from './astPrinter.ts';
import { NotSupportedYetError, TranspileFailedError } from './errors.ts';
import { safeGetNodeType, safeGetNodeTypeInCtx } from './nodeTypeProcessing.ts';
  ...
  switch (kind) {
    ...
    case LiteralKind.UnicodeString: {
      ...
      for (let index = 0; index < value.length; index++) {
        encodedData.push(value.charCodeAt(index));
      }
...
export function generateExpressionTypeStringForASTNode(
  ...
  if (node instanceof TupleExpression) {
    ...
    if (node.vComponents.length === 0) {
      ...
      return `tuple(${(type as TupleType).elements
