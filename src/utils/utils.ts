...
import { isSane } from './astChecking.ts';
...
import {
  ...
  createBytesTypeName,
  createNumberLiteral,
} from './nodeTemplates.ts';
import { isDynamicArray, isDynamicCallDataArray, safeGetNodeType } from './nodeTypeProcessing.ts';
import { Class } from './typeConstructs.ts';
...
export function traverseParent(
  ...
  const dirs = Deno.readDirSync(directory);
  const files: string[] = []
  for(let dir of dirs){
    files.push(dir.name)
  }
...
export function traverseChildren(
  ...
  if (levels === 0) {
    ...
    for (const file of files) {
      ...
      if (Deno.statSync(child).isDirectory) {
