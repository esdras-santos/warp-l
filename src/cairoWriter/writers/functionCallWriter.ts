...
import { getDocumentation, getInterfaceNameForContract } from '../utils.ts';
import { interfaceNameMappings } from './sourceUnitWriter.ts';
import { TranspileFailedError } from '../../utils/errors.ts';
import { safeGetNodeType, isDynamicArray } from '../../utils/nodeTypeProcessing.ts';
          ...
          `${
            node.vReferencedDeclaration && node.vReferencedDeclaration instanceof StructDefinition
              ? node.vReferencedDeclaration
                ? func /*mangleStructName(node.vReferencedDeclaration)*/
                : func
              : func
          }(${args})`,
        ];
