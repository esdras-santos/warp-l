...
import { SupportedSolcVersions, nethersolcPath, fullVersionFromMajor } from './nethersolc.ts';
...
function compileSolFilesCommon(files: string[], options: CompilationOptions): SolcOutput {
  ...
  const solcOutput = cliCompile(files, sources[0], options);
  return solcOutput;
}
...
function getSolFileVersion(file: string): string {
  const content = new TextDecoder("utf-8").decode(Deno.readFileSync(file))
...
function cliCompile(
  ...
  const p = Deno.run({
    cmd: [`${solcCommand} ${commandOptions}`, input[0]],
    stdio: 'piped',
    stdout: 'piped',
    stdin: 'null',
    stderr: 'piped' 
  ...
  p.status().then(() => {
    p.output().then((out)=>{
      output = new TextDecoder().decode(out);
    })
...
export function compileSolFilesAndExtractContracts(file: string): unknown {
  ...
  const solcOutput = cliCompile([file], requiredSolcVersion);
