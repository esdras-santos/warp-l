...
import { forAllWidths, getIntOrFixedByteBitWidth, mask, WarplibFunctionInfo } from '../../utils.ts';
...
export function functionaliseExp(node: BinaryOperation, unsafe: boolean, ast: AST) {
  ...
  assert(
    ...
    `${printNode(node)} has rhs-type ${(rhsType as IntType).pp()}, which is not compatible with **`,
  );
  const fullName = [
    'warp_',
    'exp',
    (rhsType as IntType).nBits === 256 ? '_wide' : '',
    (retType as IntType).signed ? '_signed' : '',
    unsafe ? '_unsafe' : '',
  ...
  const importName = [
    ...WARPLIB_MATHS,
    `exp${(retType as IntType).signed ? '_signed' : ''}${unsafe ? '_unsafe' : ''}`,
  ];
